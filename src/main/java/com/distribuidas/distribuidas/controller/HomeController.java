...
public class HomeController {
	...
	public @ResponseBody String login(@RequestBody Map<String, String> request)
		...
		String nombre = request.get("username");
	...
	public @ResponseBody String registrar(@RequestBody Map<String, String> request)
			throws JsonProcessingException, CambioPasswordException, UsuarioException {
		ObjectMapper mapper = new ObjectMapper();
		String nombre = request.get("username");
		String password = request.get("password");
		System.out.println(nombre + " " + password);
		Controlador.getInstancia().altaUsuario(nombre, password);
		return mapper.writeValueAsString("Usuario logueado");
	}
	...
	public @ResponseBody String altaProducto(@RequestBody ProductoView producto)
		...
		ObjectMapper mapper = new ObjectMapper();
		ProductoView prodAgregado = Controlador.getInstancia().altaProducto(producto);
		return mapper.writeValueAsString(prodAgregado);
	}
	...
	public @ResponseBody String agregarProductoEnPedido(@PathVariable int numero, @PathVariable int id,
			@PathVariable int cantidad) throws PedidoException, ProductoException, JsonProcessingException {
		ObjectMapper mapper = new ObjectMapper();
